...
interface EditInviteFormProps {
  invite: Invite | null;
  onClose: () => void;
  onSubmit: (formData: InviteResponse | Invite) => Promise<void>;
  isTemplate?: boolean;
}
...
export function EditInviteForm({
  ...
  const [formData, setFormData] = useState<InviteResponse | Invite | null>(
    null
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `${isTemplate ? "Template" : "Invite"} updated successfully`
      );
    } catch (error) {
      toast.error(`Failed to update ${isTemplate ? "template" : "invite"}`);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            {isTemplate ? "Edit Template" : "Edit Invite"}
          </h3>
          <button
            onClick={onClose}
            ...
            <>
              ...
              <div>
                ...
                <select
                  name="location"
                  id="location"
                  value={(formData as Invite).location || ""}
                  onChange={handleChange}
            ...
            <>
              <div>
                ...
                <input
                  ...
                  name="template_name"
                  id="template_name"
                  value={(formData as Invite).template_name || ""}
                  onChange={handleChange}
                  className="block w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 sm:text-sm"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 bg-gray-50 rounded-lg p-4 border border-gray-200">
              {Object.entries(formData.events).map(([event, checked]) => {
                // Only show relevant events based on location
                const location = (formData as Invite).location;
                if (
                  (location === "houston" && event === "coloradoReception") ||
                  (location === "colorado" && event !== "coloradoReception")
                ) {
                  return null;
                }
